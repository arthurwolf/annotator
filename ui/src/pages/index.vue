  ...
  <v-container>
    <v-card class="mx-auto my-8" elevation="16" max-width="1024">
      <v-card-item>
          ...
          <v-container>
            ...
            <v-row align="center" justify="center">
              <v-col cols="auto">
                <v-tooltip
                  location="bottom"
                  text="Upload an Asciinema recording file."
                >
                  <template v-slot:activator="{ props }">
                    <v-btn
                      ...
                      icon="mdi-upload"
                      size="large"
                      @click="on_upload_click"
                      :disabled="console_player?.loaded.value"
                    >
                    </v-btn>
                  </template>
                </v-tooltip>
              ...
              <v-col cols="auto" v-show="file_loaded">
                <v-btn
                  icon="mdi-play"
                  size="large"
                  @click="console_player.play()"
                  :disabled="console_player?.playing.value"
                >
              ...
              <v-col cols="auto" v-show="file_loaded">
                <v-btn
                  ...
                  size="large"
                  @click="console_timeline.new_annotation()"
                >
                  <v-icon icon="mdi-tag-plus"></v-icon>
                  <v-tooltip activator="parent" location="bottom">
                    Create a new annotation based on the set beginning (A) and
                    ending (B) keyframes/positions.
                  </v-tooltip>
                </v-btn>
              </v-col>
              ...
              <v-col cols="auto" v-show="file_loaded">
                <v-btn
                  ...
                  :disabled="console_timeline.is_mode_pan"
                  size="large"
                  @click="console_timeline.set_mode('pan')"
                >
              ...
              <v-col cols="auto" v-show="file_loaded">
                <v-btn
                  ...
                  :disabled="console_timeline.is_mode_zoom"
                  size="large"
                  @click="console_timeline.set_mode('zoom')"
                >
                  <v-icon icon="mdi-magnify-plus-outline"></v-icon>
                  <v-tooltip activator="parent" location="bottom">
                    Set the timeline into <strong>zoom</strong> mode.<br />
                    <strong>Click</strong> to zoom in,
                    <strong>ctrl+click</strong> to zoom out.<br />
              ...
              <v-col cols="auto" v-show="file_loaded">
                <v-btn
                  icon="mdi-pan"
                  :disabled="console_timeline.is_mode_non_interactive_pan"
                  size="large"
                  @click="console_timeline.set_mode('nonInteractivePan')"
                >
                  <v-icon icon="mdi-pan"></v-icon>
                  <v-tooltip activator="parent" location="bottom">
                    Set the timeline into <strong>non-interactive</strong> (no
                    selection) <strong>pan</strong> mode.
                  </v-tooltip>
                </v-btn>
              </v-col>
              ...
              <v-col cols="auto" v-show="file_loaded">
                <v-btn
                  ...
                  :disabled="console_timeline.is_mode_selection"
                  size="large"
                  @click="console_timeline.set_mode('selection')"
                >
              ...
              <v-col cols="auto" v-show="file_loaded">
                <v-btn
                  icon="mdi-download"
                  size="large"
                  @click="console_timeline.download()"
                >
        ...
        <v-card-title v-show="file_loaded"> Annotation text. </v-card-title>
        ...
        <v-card-text v-show="file_loaded">
          <TimelineControl
            v-if="console_timeline"
            :console_timeline="console_timeline"
          />
          <!-- 
                    -->
        </v-card-text>
      ...
      <v-card-item style="width: 100%">
        ...
        <v-card-text style="width: 100%">
          <ul>
            <li>
              <strong><pre> A </pre></strong> key: set marker A.
            </li>
            <li>
              <strong><pre> B </pre></strong> key: set marker B.
            </li>
            <li>
              <strong><pre> C </pre></strong> key: create new annotation.
            </li>
            <li>
              <strong><pre> Space </pre></strong> key: toggle play/pause.
            </li>
            <li>
              <strong><pre> Delete </pre></strong> key: delete annotation at
              current position in current row.
            </li>
          </ul>
        </v-card-text>
      </v-card-item>
...
async function read_as_text(file: File): Promise<string> {
  ...
  const readPromise = new Promise<string | ArrayBuffer | null>(
    (resolve, reject) => {
...
onMounted(async () => {
  ...
  console_timeline.on_selection((selection) => {
    ...
    // If there is no selection, return.got 
    if (!selection) {
      textarea.value = "";
      return;
    }
